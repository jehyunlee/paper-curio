...
citedby-topic-msg = Enter a topic to filter citing papers.\n\n· With a topic — an LLM selects relevant papers and writes 5W1H summaries.\n· Leave empty — citing papers are clustered automatically into a theme x year table.
citedby-register-title = Citedby — register in Zotero
citedby-register-msg = Register { $n } citing papers into Zotero? They go to the library root (Unfiled), not into any collection — file them yourself. Existing items (matched by DOI / arXiv / title) are skipped.
...
citedby-pdf-title = Citedby — download PDFs
citedby-pdf-msg = Find and attach PDFs for the { $n } newly added items? Uses Zotero's 'Find Available PDF' (DOI → URL → Unpaywall). Paywalled papers may have none; expect ~1s per item.
toast-citedby-pdf-running = Finding PDFs… { $done }/{ $total } (attached { $attached })
toast-citedby-pdf-done = PDFs done — attached { $attached } / unavailable { $missing } / errors { $failed }
toast-citedby-pdf-fail = PDF attachment failed: { $err }
