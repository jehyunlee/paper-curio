...
| `originality.md` | TS (원본 로직 포팅) | rule-based → `title. essence` |
| `index.html` | TS (`review_to_html.py` 포팅 + Audio Overview) | connections 주입 위해 TS |
| 연관 논문(connections) | TS (단일 논문 LLM 판정) | 코퍼스 임베딩 불요 |
...
- ✅ 우클릭 단일/다중 처리 + 진행 윈도우
- ✅ text·figure·review를 paper-curation 원본 함수로 생성 (py312 브리지)
- ✅ originality.md, index.html(Audio Overview 포함), 연관 논문(connections), topic 부여
- ✅ 3-provider 폴백, 비파괴 덮어쓰기 정책, ko/en 로케일
- ⏳ connections를 원본 임베딩(SPECTER2/코퍼스) 방식으로 — 현재는 단일 논문 LLM 근사
